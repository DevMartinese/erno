...
- **Twist**: a 3×3 molded with a continuous 90° twist: top and bottom
  ...
  are kite-shaped tiles. `U D E` turns keep the
  silhouette coherent, and the sides do not turn at all: a vertical slab is
  a twisted chunk of material, and ninety degrees about a horizontal axis
  lands it where nothing of that shape fits. `clearance: true` is the
  engine asking the question your hand asks, and `legalMoves()` answers
  `U D` and their primes and doubles.
...
Blocking asks whether a layer comes back to itself on the grid, which is what
glue needs. A moulded body asks something else: whether the turn has room.

```js
new Twist().legalMoves()   // U U' U2 D D' D2, and nothing else
new Twist().move('R')      // throws: it would pass through the rest
```

The Twist is wrung about its vertical axis, so a vertical slab is a twisted
chunk of material. Turned ninety degrees about a horizontal axis it lands
where nothing of that shape fits, and on the real puzzle your hand simply
stops. `clearance: true` is the engine asking the question the hand asks: it
places the layer where the turn would put it and checks whether anything that
moved now sits inside anything that did not.

It is off unless a definition asks for it, and it has to be. A Mirror cube's
blocks stick out into free air on every turn and would fail any test that
demanded a layer look like its own socket, but nothing of it ever passes
through anything, so it has clearance for all six faces. So do the Fisher, the
Ghost, the Axis, the Windmill and the Penrose. The Twist is the one puzzle in
the catalogue whose shape refuses a turn its mechanism would otherwise allow.

The two laws are separate questions and neither implies the other: clearance
lets a Siamese make the turns that would tear it, and blocking lets the Twist
make the turns that would drive it through itself.
