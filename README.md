...
The twenty-nine are not all equal citizens. **Cubes, cuboids, and cubes
fused into one** (Siamese, Fused) are the priority: they are what the
algebra, the permutation tools and the pattern game are aimed at, what a
new feature is proven on first, and what a design decision favours when it
has to choose. The minxes, the pyramids, the corner turners and the shape
mods are real, tested, and kept working. They are just not who this
library is for first.
...
ex.map("AD")   // "BU", prime-free: a 180° rotation, not a mirror
```

## Carving a live board
...
// one corner gone, every other piece exactly where it was
```

A token whose layer would grab nothing is not a move: `canMove` and
