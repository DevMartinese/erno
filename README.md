...
- **Twist**: an ordinary 3×3 whose SHAPE is wrung. The cuts are the plain
  straight ones and the mechanism underneath is a 3×3 exactly: 26 pieces, 54
  facelets, one colour per face, full cube notation, and a state a solver
  could take. What is twisted is the outside, by a rotation about the
  vertical that grows with height, so the top and bottom squares stay square
  and the columns between them spiral. It never shape-shifts and never loses
  a sticker, because the twist is a way of LOOKING at the cube rather than a
  change to it:

  ```js
  import { Cube, twist } from 'erno.js'
  new Cube({ size: 3, deform: twist(90) })  // the twisted cube, spelled out
  new Cube({ size: 4, deform: twist(45) })  // and it composes with any cube
  ```

  `deform` takes a matrix or a function. A function bends the picture point
  by point, which a matrix cannot do, and it is applied last, so the puzzle
